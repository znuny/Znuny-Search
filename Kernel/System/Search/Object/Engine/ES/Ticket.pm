...
sub ExecuteSearch {
    ...
    my $FulltextQuery = $Self->DefaultFulltextQueryBuild(
        ...
        Settings      => $ConfigObject->Get('SearchEngine::ES::TicketSearchFields')->{FulltextSettings},
        Simple        => 0,
    );
            ...
            if ( IsArrayRefWithData( $Data{$Entity}->{FulltextFields} ) ) {
                ...
                if ( !$EntitiesPathMapping{$Entity}->{Nested} ) {
                    if (
                        $Entity eq 'Ticket'
                        && $Param{Settings}
                        && $Param{Settings}->{TicketNumberSearchByPrefix}
                        && IsArrayRefWithData( $Data{$Entity}->{FulltextFields} )
                        )
                    {

                        my $ContainsTicketNumber
                            = grep { $_ eq 'TicketNumber.keyword' } @{ $Data{$Entity}->{FulltextFields} };

                        if ($ContainsTicketNumber) {
                            my $TicketNumberRaw = $TicketObject->GetTNByString($FulltextValue);

                            if ($TicketNumberRaw) {
                                push @FulltextQuery, {
                                    query_string => {
                                        fields           => ['TicketNumber.keyword'],
                                        query            => "*$TicketNumberRaw*",
                                        default_operator => $FulltextQueryOperator,
                                    }
                                };
                            }
